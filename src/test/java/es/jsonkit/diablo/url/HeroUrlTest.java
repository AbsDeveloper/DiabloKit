...
public class HeroUrlTest {
	...
	public void test() {
		...
		hero.setId(HeroUrlTest.HERO_ID);

		String buildUrl = this.generateUrl(hero);

		HeroUrlTest.LOGGER.info("Url Esperada   --> " + HeroUrlTest.EXPECTED_URL);
		HeroUrlTest.LOGGER.info("Url Construida --> " + buildUrl);

		Assert.assertEquals(HeroUrlTest.EXPECTED_URL, buildUrl);
