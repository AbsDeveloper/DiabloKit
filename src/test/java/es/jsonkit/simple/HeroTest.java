...
public class HeroTest {
	...
	public void test() {
		...
		try {
			...
			if (hs == null) {
				HeroTest.LOGGER.error("HeroTest --> El servicio no ha devuelto informacion");
				Assert.fail();
			}
			else {
				HeroTest.LOGGER.info(hs.toString());
			}
		}
		catch (IOException ioe) {
			HeroTest.LOGGER.error("HeroTest --> La informacion no pudo ser recogida, servicio no disponible " + ioe.getMessage());
			Assert.fail();
		}
